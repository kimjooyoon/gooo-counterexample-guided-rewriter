...
Every scenario declaration is additionally assigned exactly one proof choice (`FOUNDATION`, `COHERENCE`, or `REGRESSION`) and one indicator class (`DRIVER`, `OUTCOME`, or `GUARDRAIL`), with four cells in each family and class. These values are copied into typed IR, the independent evaluator result, candidate dossiers, case reports, and the semantic-metrics dossier. The four UNKNOWN cells emit their complete six-field records. Actions build/test/wall/resource/cache observations are integers when observed; a missing observation is null and accompanied by an UNKNOWN record. Improvement remains null/UNKNOWN without an identity-matched before/after pair.
