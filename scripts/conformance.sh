...
jq -e '.schema == "gooo/counterexample-guided-rewriter/conformance/v1" and .scenarios == 12 and .cells == 12 and .closed == 4 and .unknown == 4 and .refuted == 4 and .proof_counts == {"CLOSED":4,"UNKNOWN":4,"REFUTED":4} and .indicator_counts == {"CLOSED":4,"UNKNOWN":4,"REFUTED":4} and ([.cases[] | select(.pass == false)] | length) == 0 and (.paired_indicator_vector | length) == 12 and .external_utility_unknown == 1 and .repository_writes == 0 and .local_test_executions == 0 and .cross_project_required_gates == 0' "$output_dir/conformance-report.json" >/dev/null
